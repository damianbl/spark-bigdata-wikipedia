...
//    )
  ...
  def makeIndex(
    ...
    rdd
      .flatMap { article =>
        langs.map { lang =>
          (lang, if (article.mentionsLanguage(lang)) Some(article) else None)
        }
      }
      .groupByKey
      .map {
        case (lang, maybeArticles) =>
          (lang, maybeArticles.collect { case Some(article) => article })
      }
